...
## Git and GitHub — the owner does all of it

All work happens as files in the local working folder. Nothing you do leaves this
machine.

**Never run** `git commit`, `git add`, `git push`, `git pull`, `git branch`,
`git checkout`, `git merge`, `git rebase`, `git stash`, `git tag`, `gh`, or any
call to the GitHub API. **Never stage anything.** Never obtain, read or use a
GitHub token — not via `git credential fill`, not any other way.

Your job ends when the files are correct on disk. Then:

- list every file you created, changed or deleted, and what changed in each
- propose a commit message in prose — what changed and why, with the evidence
  where a claim is made
- if the work spans several logical units, propose several messages and say
  which files belong to each
- **stop**

The owner reviews the working tree, stages, commits and pushes — once at the end
of each build stage, and at any point they choose in between. If you believe
something should be committed before you continue, say so and wait. Do not do it.

**Stop and report at every module boundary**, not only at the end of a stage.
Without commits there are no checkpoints, so those report points are the owner's
only opportunity to create one.

**DVC is different and is allowed.** `dvc add` writes pointer files locally and
`dvc push` uploads data to the owner's Drive; neither touches GitHub or the commit
history. Run them when the work calls for it and report that you did. Never
`git add` the resulting `.dvc` files — list them like any other changed file.

**The one exception** is a GitHub Actions workflow committing back to the repo:
`archive.yml` writing `state/` and DVC pointers, and later `daily.yml`. That is
the deployed system running on a schedule, not you, and PLANNING 5h and 14
require it. It commits as `gridcast-archive[bot]`.
